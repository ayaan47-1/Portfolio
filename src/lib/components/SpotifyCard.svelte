...
<div class="spotify-card card-glow rounded-xl p-6 flex flex-col gap-4 backdrop-blur-sm" style="background-color: rgb(var(--color-surface-channels) / 0.6);">
	...
	<div class="flex items-center gap-2 w-full">
		...
		<!-- Live Activity Pulse (only when playing) -->
		{#if track?.isPlaying}
	...
	{:else if track}
		...
		>
			...
			<div class="flex flex-col min-w-0">
				...
				{:else}
					<!-- Time ago -->
					<span class="text-xs text-text-muted mt-0.5 font-mono">{timeAgo(track.playedAt)}</span>
				{/if}
			</div>
		</a>
	{:else}
...
<style>
	...
	.animate-music-bar-2 {
		animation: music-bar 1.1s ease-in-out infinite 0.1s;
	}
	.animate-music-bar-3 {
		animation: music-bar 0.9s ease-in-out infinite 0.2s;
	}
	.animate-music-bar-4 {
