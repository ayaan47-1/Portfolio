...
<button
	onclick={scrollToTop}
	class="fixed bottom-20 right-6 md:bottom-6 z-50 p-3 rounded-full bg-surface/80 backdrop-blur-sm border border-border text-text-secondary hover:text-accent hover:border-accent shadow-lg transition-all duration-300 transform {showScroll
...
<section id="home" class="pt-16 pb-8 md:pt-20 md:pb-12">
	<div class="mx-auto max-w-4xl px-6 md:px-8">
		...
		<div class="flex flex-wrap gap-6 text-base font-medium mb-8">
			...
			>
				<svg
					class="w-5 h-5 text-accent group-hover:text-accent-glow transition-colors"
					fill="none"
					stroke="currentColor"
					viewBox="0 0 24 24"
...
<div class="max-w-4xl mx-auto px-6 md:px-8 py-4">
	<div
		class="h-px bg-gradient-to-r from-transparent via-accent to-transparent opacity-80 shadow-[0_0_10px_theme('colors.accent')]"
	></div>
</div>
...
<Section title="About Me" id="about">
	<div class="space-y-8">
		...
		<!-- Education Card -->
		<article
			class="card-glow rounded-xl p-6 md:p-8 backdrop-blur-sm text-text-primary"
			...
			{#if personalInfo.education}
				<div>
					...
					<div class="flex flex-col md:flex-row md:items-start md:justify-between mb-4 gap-2">
								...
								<div class="mt-4 pt-4 border-t border-white/5 w-full">
									...
									<div class="flex flex-wrap gap-2">
										{#each personalInfo.education.relevantCoursework as course}
											<span
												class="px-2 py-1 text-xs font-medium bg-surface/50 border border-white/10 rounded-md text-text-secondary hover:text-accent hover:border-accent/30 transition-colors cursor-default"
											>
												{course}
											</span>
										{/each}
						...
						<div
							class="text-sm text-text-muted font-mono md:text-right mt-1.5 shrink-0 whitespace-nowrap"
						>
		...
		<article
			id="work-experience"
			class="card-glow rounded-xl p-6 md:p-8 backdrop-blur-sm text-text-primary scroll-mt-24"
			style="background-color: rgb(var(--color-surface-channels) / 0.8);"
		>
			...
			{#each workExperience as experience, index}
				...
				<div>
					<div class="flex flex-col md:flex-row md:items-start md:justify-between mb-4 gap-2">
						<div>
							<h3
								class="font-futura text-xl md:text-2xl font-semibold text-text-primary group-hover:text-accent-glow transition-colors duration-300"
							>
								{experience.position}
							</h3>
							<p class="text-accent font-medium text-base md:text-lg mt-1">
	...
	<ol class="space-y-5">
		{#each systemsThinking as item, index}
			<li class="flex gap-4 group slide-up delay-{index * 100}">
				<span
					class="font-mono text-accent-glow font-bold min-w-[3rem] text-lg group-hover:text-accent transition-colors"
				>
